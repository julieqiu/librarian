...
func TestRunPostProcessor_Owlbot(t *testing.T) {
	...
	if err := runPostProcessor(t.Context(), &config.Config{Language: config.LanguageNodejs}, library, "", repoRoot, outDir); err != nil {
		t.Fatal(err)
	}
...
func TestGenerate(t *testing.T) {
	...
	for _, library := range libraries {
		if err := Generate(t.Context(), &config.Config{Language: config.LanguageNodejs, Repo: "googleapis/google-cloud-node"}, library, absGoogleapisDir); err != nil {
			t.Fatal(err)
		}
	}
