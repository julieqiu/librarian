...
func TestVerboseFlag(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name:              "prepare with -v flag",
			args:              []string{"prepare", "-v"},
			expectDebugLog:    true,
			expectDebugSubstr: "prepare command verbose logging",
		},
		{
			name:           "prepare without -v flag",
			args:           []string{"prepare"},
			expectDebugLog: false,
		},
		{
			name:              "release with -v flag",
			args:              []string{"release", "-v"},
			expectDebugLog:    true,
			expectDebugSubstr: "release command verbose logging",
		},
		{
			name:           "release without -v flag",
			args:           []string{"release"},
			expectDebugLog: false,
		},
	} {
