...
//
// Bool fields that omit `omitempty` are required by gcloud's YAML parser to be
// present even when false; do not add omitempty to them.
type Command struct {
...
type Default struct {
	// Value is the default value to emit; it is only used when Set is true.
	Value any
	// Set reports whether a default value was explicitly specified.
	Set bool
}

// IsZero reports whether d holds no value. It satisfies yaml.IsZeroer so that
// omitempty skips unset defaults.
func (d Default) IsZero() bool {
	return !d.Set
}

// MarshalYAML implements yaml.Marshaler so that the underlying value is
// emitted directly when the default is set. Returning (nil, nil) omits the
// field.
