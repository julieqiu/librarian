...
// buildContext holds the shared state needed to build a gcloud command.
...
func NewCommand(method *api.Method, overrides *Config, model *api.API, service *api.Service) (*Command, error) {
	...
	args, err := newArguments(bc)
	if err != nil {
		return nil, err
	}
	cmd.Arguments = args
	cmd.Request = newRequest(bc)
...
func newArguments(bc *buildContext) (Arguments, error) {
	args := Arguments{}
	if bc.method.InputType == nil {
		return args, nil
	}

	for _, field := range bc.method.InputType.Fields {
		// TODO(https://github.com/googleapis/librarian/issues/3413): Improve error handling strategy (Error vs Skip) and messaging.
		if err := addFlattenedParams(field, field.JSONName, &args, bc); err != nil {
...
func addFlattenedParams(field *api.Field, prefix string, args *Arguments, bc *buildContext) error {
	if shouldSkipParam(field, bc.method) {
		return nil
	}

	if isPrimaryResource(field, bc.method) {
		param := newPrimaryResourceParam(field, bc)
	...
	if field.MessageType != nil && !field.Map {
		for _, f := range field.MessageType.Fields {
			if err := addFlattenedParams(f, fmt.Sprintf("%s.%s", prefix, f.JSONName), args, bc); err != nil {
				return err
			}
		}
		return nil
	...
	param, err := newParam(field, prefix, bc)
	if err != nil {
		return err
	}
	args.Params = append(args.Params, param)
...
func newParam(field *api.Field, apiField string, bc *buildContext) (Param, error) {
	...
	if field.ResourceReference != nil {
		spec, err := newResourceReferenceSpec(field, bc)
		if err != nil {
			return Param{}, err
		}
	...
	if isUpdate(bc.method) && param.Repeated {
		param.Clearable = true
	}

	if rule := findFieldHelpTextRule(field, bc.overrides); rule != nil {
...
func newPrimaryResourceParam(field *api.Field, bc *buildContext) Param {
	resource := getResourceForMethod(bc.method, bc.model)
	...
	hostParts := strings.Split(bc.service.DefaultHost, ".")
	shortServiceName := hostParts[0]

	param := Param{
		HelpText:          helpText,
		IsPositional:      !isList(bc.method),
...
func newResourceReferenceSpec(field *api.Field, bc *buildContext) (*ResourceSpec, error) {
	for _, def := range bc.model.ResourceDefinitions {
...
func newAttributesFromSegments(segments []api.PathSegment) []Attribute {
	...
	for i, part := range segments {
		...
		// (e.g., "projects" -> "projectsId") following gcloud convention.
...
func newRequest(bc *buildContext) *Request {
	req := &Request{
		APIVersion: apiVersion(bc.overrides),
		Collection: newCollectionPath(bc.method, bc.service, false),
	}
	...
	if len(bc.method.PathInfo.Bindings) > 0 && bc.method.PathInfo.Bindings[0].PathTemplate.Verb != nil {
		req.Method = *bc.method.PathInfo.Bindings[0].PathTemplate.Verb
	} else if !isStandardMethod(bc.method) {
		// We treat any non-standard method as a custom method (AIP-136).
		commandName, _ := getCommandName(bc.method)
...
func newAsync(bc *buildContext) *Async {
	async := &Async{
		Collection: newCollectionPath(bc.method, bc.service, true),
	}
	...
	responseTypeID := bc.method.OperationInfo.ResponseTypeID
	...
	singular := getSingularResourceNameForMethod(bc.method, bc.model)
...
func newOutputConfig(bc *buildContext) *OutputConfig {
	if !isList(bc.method) {
		return nil
	}

	resourceMsg := findResourceMessage(bc.method.OutputType)
	if resourceMsg == nil {
		return nil
	}
...
func findHelpTextRule(method *api.Method, overrides *Config) *HelpTextRule {
	return findRule(overrides, strings.TrimPrefix(method.ID, "."), func(h *HelpTextRules) []*HelpTextRule {
		return h.MethodRules
	})
}
...
func findFieldHelpTextRule(field *api.Field, overrides *Config) *HelpTextRule {
	return findRule(overrides, field.ID, func(h *HelpTextRules) []*HelpTextRule {
		return h.FieldRules
	})
}

// findRule searches for a help text rule matching selector in the given rule slice.
func findRule(overrides *Config, selector string, rules func(*HelpTextRules) []*HelpTextRule) *HelpTextRule {
	...
	for _, a := range overrides.APIs {
		...
		for _, rule := range rules(a.HelpText) {
