...
func buildNodejsLibraries(repoPath, googleapisDir string) ([]*config.Library, error) {
	...
	for _, entry := range entries {
		...
		// Keep per-library config files that exist. These are generated by
		// gapic-generator-typescript but are redundant in the monorepo. We
		// preserve them during migration to avoid noisy diffs.
