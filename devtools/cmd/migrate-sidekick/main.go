...
func main() {
	...
	if err := librarian.RunFmt(true, *output); err != nil {
...
func convertDefaultConfig(sc *sidekickconfig.Config) *config.Default {
	...
	if sc.Release != nil {
		d.Release.Remote = sc.Release.Remote
		d.Release.Branch = sc.Release.Branch
	}
...
func convertLibrary(repoPath, sidekickPath string, sc *sidekickconfig.Config, rootConfig *sidekickconfig.Config) (*config.Library, error) {
	...
	if cargoPkg.Publish != nil && !*cargoPkg.Publish {
		lib.Publish = &config.LibraryPublish{Disabled: true}
	}

	// Set specification format and source.
	if sc.General.SpecificationFormat == "disco" {
		lib.SpecificationFormat = "discovery"
		lib.SpecificationSource = sc.General.SpecificationSource
		// Discovery APIs always need explicit output since they don't have a channel.
		lib.Output = relPath
	} else if sc.General.SpecificationFormat != "" {
		lib.SpecificationFormat = sc.General.SpecificationFormat
	}
	...
	if lib.SpecificationFormat == "" && sc.General.SpecificationSource != "" {
		derivedChannel := strings.ReplaceAll(lib.Name, "-", "/")
		if sc.General.SpecificationSource != derivedChannel {
			lib.Channel = sc.General.SpecificationSource
		}
	}

	// Set service config.
	if sc.General.ServiceConfig != "" {
		lib.ServiceConfig = sc.General.ServiceConfig
	}
	...
	if len(sc.CommentOverrides) > 0 && lib.Rust == nil {
	...
	// Set release level.
	if level, ok := sc.Codec["release-level"]; ok {
		lib.ReleaseLevel = level
	}

	// Convert extra-modules to generate.keep.
	if v, ok := sc.Codec["extra-modules"]; ok && v != "" {
		if lib.Generate == nil {
			lib.Generate = &config.LibraryGenerate{}
		}
		for _, mod := range strings.Split(v, ",") {
			lib.Generate.Keep = append(lib.Generate.Keep, "src/"+mod+".rs")
		}
	}
