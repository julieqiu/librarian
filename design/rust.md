...
```bash
library=... # e.g. google-cloud-websecurityscanner-v1
```
...
V=$(cat .librarian-version.txt)
# The update command handles updating all sources, including discovery if configured.
go run github.com/googleapis/librarian/cmd/librarian@${V} update --all && \
    go run github.com/googleapis/librarian/cmd/librarian@${V} generate --all && \
    cargo fmt
...
```bash
V=$(cat .librarian-version.txt)
go run github.com/googleapis/librarian/cmd/librarian@${V} generate --all && cargo fmt
```
...
```bash
librarian update --all && librarian generate --all && cargo fmt
```
...
directory. In this case you might need to manually edit `librarian.yaml` first.

1. Add the library to `librarian.yaml` manually with custom output configuration.
2. Run generation.
...
```bash
typo && cargo fmt && cargo build && cargo test && cargo doc
git add src/generated/cloud/api/types Cargo.toml Cargo.lock librarian.yaml
```
...
# Note: You generally shouldn't remove it from librarian.yaml if you intend to add it right back,
# but for a full clean test you might.
