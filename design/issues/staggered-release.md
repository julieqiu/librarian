...
1.	**Initial Trigger (Generator Update):** A generator update is merged (e.g., via `librarianops sync-sources` and `librarianops generate-all`). This results in new generated code for many libraries, creating pending release changes.

2.	**Automated Staggered Release (`librarianops release`\):** The `librarianops release` automation is configured to run on a schedule (e.g., daily) with the `--limit` flag.
